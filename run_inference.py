...
def _sample_top_p(probs: torch.Tensor, p: float):
    ...
    # zero out all the the probabilities of tokens that are not selected by the top p
    probs_sort[mask] = 0.0
...
def test_inference(model: PaliGemmaForConditionalGeneration, processor: PaliGemmaProcessor, device: str, prompt: str,
                   image_file_path: str, max_tokens_to_generate: int, temperature: float, top_p: float, do_sample: bool):
    ...
    for _ in range(max_tokens_to_generate):
        outputs = model(input_ids, pixel_values, attention_mask, kv_cache)
...
def main(model_path: str, prompt: str, image_file_path: str = None, max_tokens_to_generate: int = 100,
