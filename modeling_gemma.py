...
class KVCache():
    ...
    def update(self, key_states: torch.Tensor, value_states: torch.Tensor, layer_idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # when we have not cached any key/value states for this layer
        ...
        # Return the updated key/value states
...
class GemmaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids, seq_len=None):
        # x: [bns, num_attention_heads, seq_len, head_size]
        ...
        with torch.autocast(device_type=device_type, enabled=True):
            ...
            # emb: [Bs, Seq_Len, Head_Dim]
            emb = torch.cat((freqs, freqs), dim=-1)
            # cos, sin: [Bs, Seq_Len, Head_Dim]
            cos = emb.cos()
            sin = emb.sin()
...
class GemmaAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor] = None,
        ...
        # [Bs, Seq_Len, Num_Head_Q * Head_Dim]
        query_states = self.q_proj(hidden_states)
        # [Bs, Seq_Len, Num_Head_KV * Head_Dim]
        ...
        # Repeat key and value to match the number of heads of the query
        ...
        # Multiply by W_o. [Batch_Size, Seq_Len_O, Hidden_Size]
...
class GemmaForCausalLM(nn.Module):
    ...
    def forward(self, attention_mask, position_ids, inputs_embeds, kv_cache) -> Tuple:
        ...
        logits = logits.float()  # Cast to float for numerical stability
        return_data = {
            "logits": logits
        }
        if kv_cache:
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(
        ...
        # We need to expand the masks to the embedding dimension otherwise we can't use them torch.where
        text_mask_expanded = text_mask.unsqueeze(-1).expand(-1, -1, embed_dim)
        image_mask_expanded = image_mask.unsqueeze(-1).expand(-1, -1, embed_dim)
        pad_mask_expanded = pad_mask.unsqueeze(-1).expand(-1, -1, embed_dim)
        # Add the text embeddings
        final_embedding = torch.where(text_mask_expanded, inputs_embeds, final_embedding)
        # Insert the image embeddings. We can't use torch.where because the sequence length of scaled_image_features is not equal to the sequence length of the final_embedding
        ...
        if kv_cache or kv_cache.num_items() == 0:
        ...
        if kv_cache and kv_cache.num_items() > 0:
