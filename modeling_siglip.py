...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        # Add position embeddings to the patch embeddings. Each positional encoding is a vector of size `Embed_dim`
...
class SiglipAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        attn_weights = nn.functional.softmax(atten_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
        # Apply dropout only during training
        attn_weights = nn.functional.dropout(attn_weights, p=self.dropout, training=self.training)
        # Multiply the attention weighs by the value states. attn_output: [Batch_Size, Num_Heads, Num_Patches, Head_Dim]
